function plot_result_complex_geom(results,E_set,mesh,labels)
    ...
    for i = 1:num_plots
        ...
        [E_mag,E_mag_max,E_mag_max_ind] = E_to_mag(E);
        E_mag_n = E_mag/E_mag_max;

        nexttile; hold on
        % Plot mesh
        hp = patch('Faces',mesh.faces,'Vertices',mesh.vertices,'FaceVertexCData',E_mag,'FaceColor','interp','LineStyle','none');
        % Add arrows
        quiver3(mesh.vertices(arrow_inds,1),mesh.vertices(arrow_inds,2),mesh.vertices(arrow_inds,3),E(arrow_inds,1),E(arrow_inds,2),E(arrow_inds,3),0.5,"filled",'Color',[0.70,0.70,0.70],'MaxHeadSize',1)
        ...
        [centroid,centroid_ind] = calculate_WCOG(E_mag_n,mesh.vertices);
        centroid_dir = E(centroid_ind,:);
        centroid_dir = centroid_dir/norm(centroid_dir);
        centroid_lifted = centroid + -N*0.01;
