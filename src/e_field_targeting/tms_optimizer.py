"""
This module provides the primary user-facing API for the E-Field Targeting package.

The TMSOptimizer class simplifies the process of running stimulus optimization by
encapsulating the execution of the core optimization algorithm.
"""
...
class OptimizationResult:
    """
    Holds the results of a stimulus optimization run and provides
    methods for visualization.

    An instance of this class is returned by TMSOptimizer.run_optimization().
    """

    def __init__(
        self,
        optimizer_instance: StimulusOptimizer,
        ...
        self.weights = solution_weights
        self.location_error_m = loc_error
        self.angle_error_deg = angle_error
        self._target_pos = target_pos
        self._target_dir = target_dir

    @property
    def location_error_mm(self) -> float:
        """The final location error in millimeters."""
        return self.location_error_m * 1000

    def visualize_solution(self):
        """Creates an interactive 3D plot of the optimization result using Plotly."""
        target_pos_jax = jnp.asarray(self._target_pos)
        normalized_weights = self.weights / self._optimizer.stimulator.max_current_slope
        e_combined = jnp.einsum(
            "c,cvd->vd", normalized_weights, self._optimizer.efield_set
        )
        e_mag, _, _ = _e_to_mag(e_combined)

        realized_pos, realized_dir = _get_stimulated_target_3d(
            e_combined, self._optimizer.mesh, target_pos_jax
        )

        fig = go.Figure(
            go.Mesh3d(
                x=np.array(self._optimizer.mesh.vertices[:, 0]),
                y=np.array(self._optimizer.mesh.vertices[:, 1]),
                z=np.array(self._optimizer.mesh.vertices[:, 2]),
                i=np.array(self._optimizer.mesh.faces[:, 0]),
                j=np.array(self._optimizer.mesh.faces[:, 1]),
                k=np.array(self._optimizer.mesh.faces[:, 2]),
        ...
        arrow_scale = (
            np.mean(np.ptp(np.array(self._optimizer.mesh.vertices), axis=0)) * 0.2
        )
        fig.add_trace(
            go.Cone(
                x=[self._target_pos[0]],
                y=[self._target_pos[1]],
                z=[self._target_pos[2]],
                u=[self._target_dir[0]],
                v=[self._target_dir[1]],
                w=[self._target_dir[2]],
                sizeref=arrow_scale,
                showscale=False,
        ...
        mesh_center = np.mean(self._optimizer.mesh.vertices, axis=0)
        distance_factor = np.max(np.std(self._optimizer.mesh.vertices, axis=0)) * 100
        camera_pos = (
            mesh_center + np.array(self._optimizer.mesh.average_normal) * distance_factor
        )
        camera = {
            "eye": {
                "x": camera_pos[0].item(),
...
class TMSOptimizer:
    """
    A high-level interface for running TMS stimulus optimization.

    This class handles optimizer setup and execution, providing a simplified
    workflow for the user. It expects pre-loaded data objects for initialization.
    """
    ...
    def __init__(
        ...
        """
        Initializes the optimizer with pre-loaded data structures.
        """
        print("Initializing TMS Optimizer...")
        self.jax_device = jax.devices()[0]
        print(f"Using JAX device: {self.jax_device}")

        self._stimulator = stimulator
        self._mesh = efield_model.mesh

        self._settings = settings or StimulusOptimizer.DEFAULT_SETTINGS
        self._optimizer_core = StimulusOptimizer(
            stimulator=self._stimulator,
            mesh=efield_model.mesh,
            efield_set=efield_model.efield_set,
            efield_basis=efield_model.efield_basis,
            settings=self._settings,
        )
        print("Initialization complete. Ready to run optimization.")
    ...
    def run_optimization(
        ...
        """
        Runs the optimization for a given stimulation target.
        """
        print("Starting optimization...")
        final_weights, loc_error, angle_error = self._optimizer_core.run(
        ...
        result = OptimizationResult(
            optimizer_instance=self._optimizer_core,
            solution_weights=np.array(final_weights),
        ...
        print(f"Final Location Error: {result.location_error_mm:.2f} mm")
        print(f"Final Angle Error: {result.angle_error_deg:.2f} degrees")
        return result
